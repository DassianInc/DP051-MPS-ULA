...
namespace Bryntum.Gantt.Request.Handler
{
    public class TaskSyncHandler : SyncStoreRequestHandler<Task> {
        ...
        public override IDictionary<String, Object> Update(Task task, IDictionary<String, Object> changes)
        {
            ...
            if (changes.ContainsKey("PercentDone")) task.PercentDone = Convert.ToInt32(changes["PercentDone"]);
            if (changes.ContainsKey("SchedulingMode")) task.SchedulingMode = Convert.ToString(changes["SchedulingMode"]);
            if (changes.ContainsKey("BaselinePercentDone")) task.BaselinePercentDone = Convert.ToInt32(changes["BaselinePercentDone"]);
            ...
            if (changes.ContainsKey("ConstraintDate")) task.ConstraintDate = Convert.ToDateTime(changes["ConstraintDate"]);
