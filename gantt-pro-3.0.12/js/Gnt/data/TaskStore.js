...
Ext.define('Gnt.data.TaskStore', {
    ...
    */
    ...
    // Overridden from EventStore mixin to provide Gantt model aware id consistency manager
    ...
    // Overridden from EventStore mixin to provide Gantt model aware model persistence manager
    ...
    onAssignmentMutation : function (assignmentStore, assignments) {
        ...
        Ext.Array.forEach([].concat(assignments), function (assignment) {
...
//
    ...
    onTaskStoreWrite : function (store, operation) {
        ...
        Ext.Array.forEach(records, function (task) {
    ...
    beginIndent : function (tasks) {
        this.fireEvent('beforeindentationchange', this, tasks);
    },

    endIndent   : function (tasks) {
        this.fireEvent('indentationchange', this, tasks);
    },
     ...
     */
    indent: function (tasks) {
        // TODO: method should fail (and return false?) if passed nodes are from different parent nodes
        tasks       = Ext.isArray(tasks) ? tasks.slice() : [tasks];

        tasks.sort(function(a, b) { return a.data.index - b.data.index; });

        this.beginIndent(tasks);
        this.suspendEvent('beforeindentationchange', 'indentationchange');

        Ext.Array.forEach(tasks, function(node) { node.indent(); });

        this.resumeEvent('beforeindentationchange', 'indentationchange');
        this.endIndent(tasks);
    },
     ...
     */
    outdent: function (tasks) {
        // TODO method should fail (and return false?) if passed nodes are from different parent nodes
        tasks       = Ext.isArray(tasks) ? tasks.slice() : [tasks];

        tasks.sort(function(a, b) { return b.data.index - a.data.index; });

        this.beginIndent(tasks);
        this.suspendEvent('beforeindentationchange', 'indentationchange');

        Ext.Array.forEach(tasks, function(node) { node.outdent(); });
