...
//                }
                ...
                if (Ext.getVersion().isGreaterThan('5.1.2')) {
                    // without forcing scrollable true on locked view config locked view may be
                    // not scrollable vertically. To reproduce disable this config and run tree example with 5.1.2
                    // also reproducible in advanced and basic gantt examples (for basic you need to set locked grid
                    // width to like 100)
                    var lockedViewConfig = this.lockedViewConfig = this.lockedViewConfig || {};
                    Ext.applyIf(lockedViewConfig, {
                        scrollable : true
                    });
                }
                ...
                if (this.crudManager) {
                    ...
                    this.mon(this.crudManager, {
                        beforeloadapply : this.suspendViewsRefresh,
                        load            : this.resumeViewsRefresh,
                        scope           : this
                    });
                }
            ...
            refreshHeaderContainerScrollable : function () {
                this.getSchedulingView().headerCt.getScrollable().doRefresh();
            },
    ...
    resumeViewsRefresh  : function (doRefresh) {
        ...
        if (doRefresh) { // both views tried to refresh
