...
Ext.define('Sch.mixin.Zoomable', {
    ...
    */
    ...
    zoomToLevel: function (level, span, options) {
        ...
        // #2439
        // We need to clear cached center date if view is scrolled, but not if that scroll happen while zooming.
        // It is possible that listener is not unbound before we zoom, unbind it here to save cached date
        view.un('scroll', me.clearCenterDateCache, me);

        // Add cache clear listener when all scrolling related to zooming is done
        view.on('scroll', function () {
            // only need to clear cache once
            view.on('scroll', me.clearCenterDateCache, me, { single : true });
        }, view, { single : true });
